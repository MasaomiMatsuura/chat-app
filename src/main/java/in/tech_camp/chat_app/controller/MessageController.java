...
public class MessageController {
  ...
  public String showMessages(@PathVariable("roomId") Integer roomId, @AuthenticationPrincipal CustomUserDetail currentUser, Model model) {
    ...
    List<RoomUserEntity> roomUserEntityies = roomUserRepository.findByUserId(currentUser.getId());
    List<RoomEntity> roomList = roomUserEntityies.stream().map(RoomUserEntity::getRoom).collect(Collectors.toList());
  ...
  public String saveMessages(@PathVariable("roomId")Integer roomId, @ModelAttribute("message")MessageForm messageForm, @AuthenticationPrincipal CustomUserDetail currentUser) {
